...
def main(argv):
	...
	original_lenghts_per_config = {}

	folds_discriminator = 'fold_'
	files_discriminator = 'flat_'
	gammas_discriminator = 'gamma_'
	inferred_discriminator = 'inferred_'

	for file in files:
		if folds_discriminator in file:
			fold = int(file.split('_')[1])

			fold_files = os.listdir(data_folder+file+'/')

			i = 0

			bar = progressbar.ProgressBar(maxval=len(fold_files), widgets=[progressbar.Bar('=', 'B', 'D'), ' ', progressbar.Percentage()])
			print('***  Extract fold '+str(fold)+'  ***')
			bar.start()

			gen = (fold_file for fold_file in fold_files if files_discriminator in fold_file)

			for fold_file in gen:
				config = '_'.join(fold_file.split('_')[:6])
				if gammas_discriminator in fold_file: # Intercept thresholded predictions

					gamma = float(fold_file.split('_')[7][:-4])

					if config not in oracles_per_config:
						oracles_per_config[config] = {}
					if fold not in oracles_per_config[config]:
						oracles_per_config[config][fold] = {}
					if gamma not in oracles_per_config[config][fold]:
						oracles_per_config[config][fold][gamma] = []

					if config not in predictions_per_config:
						predictions_per_config[config] = {}
					if fold not in predictions_per_config[config]:
						predictions_per_config[config][fold] = {}
					if gamma not in predictions_per_config[config]:
						predictions_per_config[config][fold][gamma] = []

					fin = open(data_folder+file+'/'+fold_file,'r')
					lines = fin.readlines()
					fin.close()

					for line in lines[1:]: # First line contains column names
						oracles_per_config[config][fold][gamma].append(line.split()[0])
						predictions_per_config[config][fold][gamma].append(line.split()[1])

				elif inferred_discriminator not in fold_file: # Intercept gamma 0 predictions
					config = config[:-4]

					if config not in original_lenghts_per_config:
						original_lenghts_per_config[config] = {}
					if fold not in original_lenghts_per_config[config]:
						original_lenghts_per_config[config][fold] = 0.0

					fin = open(data_folder+file+'/'+fold_file,'r')
					lines = fin.readlines()
					fin.close()

					original_lenghts_per_config[config][fold] = len(lines)-1 # One line is intestation			
	...
	bar = progressbar.ProgressBar(maxval=len(oracles_per_config), widgets=[progressbar.Bar('=', 'B', 'D'), ' ', progressbar.Percentage()])
	print('***  Compute metrics  ***')
	bar.start()

	for config in oracles_per_config:
		if config not in metrics_per_config:
			metrics_per_config[config] = {}
		for fold in oracles_per_config[config]:
			if fold not in metrics_per_config[config]:
				metrics_per_config[config][fold] = []
			for gamma in oracles_per_config[config][fold]:
				temp_metrics = Metrics()

				temp_metrics.accuracy = accuracy_score(oracles_per_config[config][fold][gamma],predictions_per_config[config][fold][gamma])
				temp_metrics.f_measure = f1_score(oracles_per_config[config][fold][gamma],predictions_per_config[config][fold][gamma],average='macro')
				temp_metrics.g_mean_multiclass = compute_g_mean_multiclass(oracles_per_config[config][fold][gamma],predictions_per_config[config][fold][gamma])
				temp_metrics.g_mean_macro = geometric_mean_score(oracles_per_config[config][fold][gamma],predictions_per_config[config][fold][gamma],average='macro')
				temp_metrics.classified_ratio = float(len(predictions_per_config[config][fold][gamma]))/float(original_lenghts_per_config[config][fold])
...
#####################################################################################
	...
	# Apply threshold to aggregated classified_ratio
