...
def main(argv):
	...
	ax = sns.heatmap(df_cm*100,vmin=0.001,vmax=100,cmap='Blues',square=True,norm=LogNorm(cm.min()*100,cm.max()*100),cbar=False)
	...
	plt.tick_params(bottom=False,top=False,left=False,right=False,labelrotation=0)
	...
	plt.savefig(image_folder+file_name[:-4]+'_confusion_matrix.eps',format='eps',dpi=1000)
