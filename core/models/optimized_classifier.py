...
class OptimizedClassifier(object):

    def __init__(self, model_class, level=0, fold=0, arbitrary_discr='', multimodal=False):
        ...
        self.model_optimizer = optimizer_fun(model, param_grid, verbose=1,
                                                        n_jobs=workers_number)
        self.file_out = 'data_%s/material/optimization_results_fold_%s.csv' % (model_class, fold)
    ...
    def fit(self, training_set, ground_truth):
        ...
        if self.n_clusters < 2:
            ...
            if self.optimize:
                t = time.time() - t
                self.model_optimizer.fit(local_training_set, ground_truth[normal_index])
                t = time.time() - t
                self.model = self.model_optimizer.best_estimator_
                import pandas as pd
                ...
                t = time.time() - t
                self.model.fit(local_training_set, ground_truth[normal_index])
                t = time.time() - t
            if hasattr(self.model, 'model_'):
                self.model_ = self.model.model_
            else:
            ...
            for i in range(self.n_clusters):
                ...
                self.models[i].fit(local_training_set[red_normal_index], ground_truth[normal_index])
            t = time.time() - t
            for model in self.models:
                if hasattr(model, 'model_'):
                    self.model_.append(model.model_)
                else:
    ...
    def predict(self, testing_set):
        ...
        if self.n_clusters < 2:
            t = time.time() - t
            pred = self.model.predict(local_testing_set)
            ...
            for i, _model in enumerate(self.models):
                ...
                pred[red_index] = _model.predict(local_testing_set[red_index])
    ...
    def predict_proba(self, testing_set):
        ...
        if self.dl and not self.multimodal:
            ...
            scores = [ad.predict_proba for ad in self.models]
        elif self.multimodal:
            local_testing_set = testing_set
            score = self.model.predict_proba
            scores = [ad.predict_proba for ad in self.models]
            ...
            scores = [ad.decision_function for ad in self.models]
    ...
    def fit_NeuralWeightsDecisor(self, training_set, ground_truth):
        ...
        decisor = AnomalyDetector(self.model_class, self.detector_opts, self.anomaly_class, self.features_number,
                                  self.epochs_number,
